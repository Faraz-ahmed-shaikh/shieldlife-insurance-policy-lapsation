...
# MAGIC **Purpose:** Explore all 8 raw tables to understand structure, 
# MAGIC schema, data quality issues, and statistical distributions 
# MAGIC before cleaning.  
# MAGIC **Input:** Raw CSV files — customers, policies,
# MAGIC products, channels, agents, date, 
# MAGIC policy_events, premium_payments 
# MAGIC **Output:** Exploration findings and proposed cleaning plan  
# MAGIC **Platform:** Databricks (PySpark)

# COMMAND ----------

# MAGIC %md
# MAGIC ### Exploring Customers Data
...
# MAGIC %md
# MAGIC ### Exploring Policies Data 

# COMMAND ----------

policies_df = spark.read.csv("/Volumes/workspace/default/shieldlife_insurance_db/original_data/policies_data.csv", header=True, inferSchema=True)
...
display(policies_df.filter((col("channel_id")!=1) & (col("agent_id").isNotNull()))) # There are no policies which arn't sold by agent network, and has a agent_id

# COMMAND ----------

display(policies_df.filter(col("policy_value")<col("cost_spent"))) # There 4,826 policies whoes cost is more than its value

# COMMAND ----------

display(policies_df.filter(col("policy_premium")<500)) # There are 592 policies which have premium less than 500 inlcuding 0s 

# COMMAND ----------

display(policies_df.filter(col("issue_date")>'2026-06-30')) # There are 647 policies with future issue dates

# COMMAND ----------

display(policies_df.filter(col("issue_date")>col("expiry_date"))) # There are 740 policies which issue dates is more than its expire

# COMMAND ----------

# MAGIC %md
# MAGIC ### Exploring fact_policy_events Data

# COMMAND ----------

fact_policy_events = spark.read.csv('/Volumes/workspace/default/shieldlife_insurance_db/original_data/policy_events_data.csv', inferSchema=True, header=True)
...
display(fact_policy_events.groupBy("is_renewed").count())
...
display(fact_policy_events.filter(col("is_renewed")==True).select("previous_status","current_status"))
...
fact_premium_payments = spark.read.csv('/Volumes/workspace/default/shieldlife_insurance_db/original_data/premium_payments_data.csv', inferSchema=True, header=True)
...
display(fact_premium_payments.groupBy("is_missed").count())
...
ghost_events_df = fact_premium_payments.join(
    policies_df,
    on="policy_id", # change this to your actual FK column name
    how="anti"
)
...
fact_premium_payments.filter(fact_premium_payments.premium_amount <0).count()